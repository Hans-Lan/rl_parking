...
def create_parking_lot(config: ParkingConfig) -> sg.MultiLineString:
    ...
    if mode == 'parallel':
        ...
        wall = sg.LineString([(-12, 6 + l), (12, 6 + l)])
...
class ParkingLots(gym.Env):
    ...
    def __init__(self, config: ParkingConfig):
        ...
        if config.mode == 'parallel':
            self.init_states_high = np.array([5 + car_l, 4 + car_w / 2, np.pi / 4])
            self.init_states_low = np.array([-5 - car_l, 1 + car_w / 2, -np.pi / 4])
            self.terminal_states = np.array([-car_l / 2 + car_d, 0, 0])
        elif config.mode == 'vertical':
            # self.init_states_high = np.array([5 + car_l, 3 + car_l / 2, np.pi / 6])
            # self.init_states_low = np.array([-5 - car_l, 1 + car_l / 2, -np.pi / 6])
            self.init_states_high = np.array([car_l, 3.5 + car_l / 2, np.pi / 12])
            self.init_states_low = np.array([car_w + 1, 2.5 + car_l / 2, -np.pi / 20])
            self.terminal_states = np.array([0, -car_l / 2 + car_d, np.pi / 2])
        else:
            raise ValueError(f'Unknown parking mode: {config.mode}')
        self.admissible_errors = np.array([0.1, 0.1, 0.1])
        self.x_limit = 12

        # action: [v, steer]
        self.action_space = gym.spaces.Box(
            low=np.array([-1, -np.pi / 5], dtype=np.float32),
            high=np.array([1, np.pi / 5], dtype=np.float32), 
        )
        ...
        self.observation_space = gym.spaces.Box(
            low=np.array([-15, -10, -1, -1], dtype=np.float32),
            high=np.array([15, 10, 1, 1], dtype=np.float32)
        )
        self.frac = 0.
        self.easy = 0
        self.seed()
    ...
    def step(self, action):
        ...
        self.state = np.array([newx, newy, newtheta])
    ...
    def jump_start_vertical(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        mode = self.np_random.choice(3, p=[0.6, 0.2, 0.2])
        if mode == 0:
            high = np.array([0.1, -car_l / 2 + car_d + 3.5, np.pi / 2 + 0.1])
            low = np.array([-0.1, -car_l / 2 + car_d + 0.2, np.pi / 2 - 0.1])
        elif mode == 1:
            high_bias = self.np_random.uniform(low=4.0, high=5.0)
            ang_bias = (high_bias - 4.0) * np.pi / 6 + 0.3
            high = np.array([0.5, -car_l / 2 + car_d + high_bias, np.pi / 2 - 0.2])
            low = np.array([0.1, -car_l / 2 + car_d + 3.5, np.pi / 2 - ang_bias])
        elif mode == 2:
            # low_bias = self.np_random.uniform(low=2.5, high=3.5)
            # angle = (3.5 - low_bias) * np.pi / 6 + 0.3
            high = np.array([2.0, car_l / 2 + 3.5, np.pi / 4])
            low = np.array([1.5, car_l / 2 + 2.5, np.pi / 12])
        return high, low
...
def env_creator(config: EnvContext):
    ...
    return TimeLimit(ParkingLots(env_config), max_episode_steps=env_config.max_step)